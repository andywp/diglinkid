...
class Pay extends Controller {
    ...
    private function paypal_create() {
		...
		//$this->settings->payment->currency='USD';
		
		

        /* Payment details */
        $product = $this->package->name;
        $price = (float) $this->package->{$_POST['payment_plan'] . '_price_usd'};
		//if discount
		$originalPrice=$price;
		if(!empty($discount)){
        ...
        switch($_POST['payment_type']) {
            case 'one-time':
                ...
                $redirectUrls->setReturnUrl(url('pay/' . $this->package_id . '/paypal-success?payment_plan=' . $_POST['payment_plan'].'&voucher='.$voucher.'&discount='.$discount))
            ...
            case 'recurring':
                ...
                $payment_definition->setName('Regular Payments')
                    ...
                    ->setAmount(new Currency(['value' => $price, 'currency' => 'USD']));


                /* Set merchant preferences */
                $merchant_preferences = new MerchantPreferences();
                  $merchant_preferences->setReturnUrl(url('pay/' . $this->package_id . '/paypal-success?payment_plan=' . $_POST['payment_plan'] . '&payment_type=recurring&voucher='.$voucher.'&discount='.$discount))
                    ->setCancelUrl(url('pay/' . $this->package_id . '/paypal-cancel' . '&payment_type=recurring'))
                    ->setAutoBillAmount('yes')
                    ->setInitialFailAmountAction('CONTINUE')
                    ->setMaxFailAttempts('0')
                    ->setSetupFee(new Currency(['value' => $price, 'currency' => 'USD']));
...
//                }
            ...
            if(isset($_GET['token'], $_GET['payment_type'])) {
               ...
               $paypal->setConfig(['mode' => $this->settings->paypal->mode]); 
				...
				echo '<pre>'; */
					...
					$payer=$agreement->payer->payer_info;
					...
					$detail=$agreement->agreement_details;
					
					/* $payment_total=$detail->value; */
					/* $payment_total=($detail->outstanding_balance->value == 0 )?$detail->value:$detail->outstanding_balance->value; */
					$payment_total=$detail->outstanding_balance->value;
					
					$payment_currency=$detail->currency;
				...
				$data=$this->database->query("SELECT name FROM packages WHERE package_id =".$this->package_id);
    ...
    private function stripe_create() {
        ...
        /* Payment details */
        $product = $this->package->name;
        $price = $this->package->{$_POST['payment_plan'] . '_price_usd'};
        $stripe_formatted_price = in_array('USD', ['MGA', 'BIF', 'CLP', 'PYG', 'DJF', 'RWF', 'GNF', 'UGX', 'JPY', 'VND', 'VUV', 'XAF', 'KMF', 'KRW', 'XOF', 'XPF']) ? number_format($price, 2) : number_format($price, 2) * 100;
        $shipping = 0;
	...
	public function cekvoucher(){
		...
		$date=date('Y-m-d');
		$now=strtotime($date);
		//SELECT * FROM `cupon` WHERE start_date <= '2020-07-27' and end_date >= '2020-07-27'
		$data=$this->database->query("SELECT * FROM cupon where start_date <= '".$date."' and end_date >= '".$date."' and cupon='".trim($_POST['voucher'])."'  ");
		$cupon=$data->fetch_object();
		if($cupon){
			$alert = 'Voucher '.$cupon->cupon.' discount '.$cupon->diskon.' successfully installed';
			$diskon=$cupon->diskon;
			$voucher=$cupon->cupon;
			$error = false;
		}else{
			$alert = 'voucher not found or expired';
		}
