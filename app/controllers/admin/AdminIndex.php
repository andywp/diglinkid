...
class AdminIndex extends Controller {
    ...
    public function index() {
        ...
        if($this->settings->payment->is_enabled) {
            $payments = Database::$database->query("SELECT COUNT(*) AS `payments`, IFNULL(TRUNCATE(SUM(`amount`), 0), 0) AS `earnings` FROM `payments` WHERE `currency` = '{$this->settings->payment->currency}'")->fetch_object();
