...
exit();	 */			
	...
	public function snaprenew(){
		$this->paymentsID = isset($this->params[0]) ? $this->params[0] : false;
		 $this->paymentsID = base64_decode($this->paymentsID);
		if(!$this->paymentsID){
			redirect(SITE_URL);
			exit();
		}
		$this->database->query("update payments set date='".date('Y-m-d h:i:sa')."', gateway_id='".$this->paymentsID."-".time()." ' where id=".$this->paymentsID);
		$data=$this->database->query("SELECT a.*, b.name as packages FROM  payments as a, packages as b 
														where 
															a.package_id=b.package_id
														and														
														...
														and
															a.note=1
														and
															( a.status !='settlement' or a.status !='capture')
															");
		...
		echo '</pre>'; 
		 */
		$view = new \Altum\Views\View('payment/snaprenew', (array) $this);
		$data = [
            'payment'    => $payments,
			'jenis'		=>  'gopay' 
        ];
	...
	public function cron_cek_gopay(){
		...
		foreach($product as $r){
			if($r['transaction_id'] !=''){
			//add	
			//$transaction_id=($r['gateway_id'] !='' )?$r['gateway_id']:$r['transaction_id'];
			...
			$cek=$this->httpGet($config['urlgopay'].$r['transaction_id'].'/status');
				...
				$id=$r['id'];
				$payments=$this->database->query('update payments set status=\''.$cek->transaction_status.'\' '.$addQuery.' where id=\''.$r['id'].'\' ');
			}
		}
...
/* 	public function mail_send($array,$via,$id,$admin=true ){
	...
	//	$data = json_decode(file_get_contents('php://input'), true);
		...
		//$this->database->query("insert into  param set  param='".serialize($data) ."'");
		$idGET=explode('-',$_GET['order_id']);
		$idGET=$idGET[0];
		...
		$dataQWR=$this->database->query("SELECT user_id,package_id,plan FROM payments WHERE id =".$idGET);		
...
/* 		 echo '<pre>';
		...
		if($data->payment_type =='credit_card'){
			...
			Database::$database->query("update payments set processor='".$data->payment_type."', status='".@$data->transaction_status."', transaction_id='".$data->transaction_id."' ".$addquery." where id='".$idGET."' ");
			...
			$data=Database::$database->query("update payments set processor='".$data->payment_type."', status='".@$data->transaction_status."', transaction_id='".$data->transaction_id."', date_paid='".@$data->settlement_time."' where id='".$idGET."' ");
			
		}
		//$this->cron_cek_gopay();
		//exit();
		redirect('invoice/'.$idGET);
		//echo 'hasdjhadhasdj';
		exit();
	}
	
	
	public function finishrenew(){
		global $config;
		...
		if($data->payment_type =='credit_card'){
			...
			$data=Database::$database->query("update payments set processor='Credit Card', status='".@$data->transaction_status."', transaction_id='".$data->transaction_id."' , date_paid='".$data->transaction_time."'  where id='".$idGET."' ");
