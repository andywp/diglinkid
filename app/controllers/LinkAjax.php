...
class LinkAjax extends Controller {
    ...
    private function create() {
        ...
        switch($_POST['type']) {
            ...
            case 'biolink':
				...
				$biolink_link_types=array_merge($biolink_link_types,['whatsapp_form']);
                /* Check for subtype */
                if(isset($_POST['subtype']) && in_array($_POST['subtype'], $biolink_link_types)) {
                    ...
                    }elseif($_POST['subtype'] == 'whatsapp_form'){
						$this->create_form_whatsapp();
					}else {
    ...
    private function create_biolink() {
        ...
        /* Insert to database */
        $stmt = Database::$database->prepare("INSERT INTO `links` (`project_id`, `user_id`, `domain_id`, `type`, `subtype`, `url`, `settings`, `date`, `subscribe`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('sssssssss', $_POST['project_id'], $this->user->user_id, $domain_id, $type, $subtype, $url,  $settings, \Altum\Date::$date,$subscribe);
        $stmt->execute();
        $link_id = $stmt->insert_id;
        $stmt->close();
	...
	private function create_form_whatsapp(){
		$_POST['link_id'] = (int) $_POST['link_id'];
        $url = trim(Database::clean_string($_POST['url']));
		$_POST['location_url']=trim($_POST['location_url']);
		//code indonesia
		if(substr($_POST['location_url'],0,2) != 62){
			$_POST['location_url']='62'.(int)$_POST['location_url'];
		}
		...
		$stmt = Database::$database->prepare("INSERT INTO `links` (`project_id`, `biolink_id`, `user_id`, `type`, `subtype`, `url`, `location_url`, `settings`, `date`, `keterangan`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('ssssssssss', $project_id, $_POST['link_id'], $this->user->user_id, $type, $subtype, $url, $_POST['location_url'], $settings, \Altum\Date::$date, $_POST['keterangan']);
        $stmt->execute();
        $stmt->close();
    ...
    private function update_biolink() {
        ...
        if($image) {
			...
			if($image_file_size > 1024000) {
                Response::json('max size 1MB', 'error');
            }
            ...
            case 'image':
                ...
                if($background) {
                    ...
                    $background_file_temp = $_FILES['background']['tmp_name'];
					 $image_file_size = $_FILES['background']['size'];
					...
					if($image_file_size > 1024000) {
						Response::json('max size 1 MB', 'error');
					}
                    /* Delete current image */
				...
				$background_mobile = (bool) !empty($_FILES['background_mobile']['name']);
				if($background_mobile) {
					 $background_file_extension = explode('.', $_FILES['background_mobile']['name']);
                    $background_file_extension = strtolower(end($background_file_extension));
                    $background_file_temp = $_FILES['background_mobile']['tmp_name'];

                    if($_FILES['background_mobile']['error']) {
                        Response::json($this->language->global->error_message->file_upload, 'error');
                    }
					if( $_FILES['background_mobile']['size'] > 1024000) {
						Response::json('Batas background mobile  ukuran 1MB', 'error');
					}
                    if(!in_array($background_file_extension, $image_allowed_extensions)) {
                        Response::json($this->language->global->error_message->invalid_file_type, 'error');
                    }
                    ...
                    /* Delete current image */
                    if(!empty($link->settings->background_mobile) && file_exists(UPLOADS_PATH . 'backgrounds/' . $link->settings->background_mobile)) {
                        unlink(UPLOADS_PATH . 'backgrounds/' . $link->settings->background_mobile);
                    }
                    ...
                    $background_mobile = $background_new_name;
				}
        ...
        $_POST['google_tag_manager'] = Database::clean_string($_POST['google_tag_manager']);
		@$_POST['category'] = Database::clean_string($_POST['category']);
		$subscribe=!empty(($_POST['subscribe']))?htmlentities($_POST['subscribe']):'';
        ...
        $settings = json_encode([
			...
			'background_mobile' => $background_mobile ? $background_mobile : $link->settings->background_mobile,
        ...
        /* Update the record */
         $stmt = Database::$database->prepare("UPDATE `links` SET `domain_id` = ?, `url` = ?, `settings` = ?, `subscribe` = ? WHERE `link_id` = ?");
        $stmt->bind_param('sssss', $domain_id, $url, $settings, $subscribe, $link->link_id);
        $stmt->execute();
        $stmt->close();
	...
	private function update_whatsapp_form(){
		$_POST['link_id'] = (int) $_POST['link_id'];
        $url = trim(Database::clean_string($_POST['url']));
		$_POST['location_url']=trim($_POST['location_url']);
		//code indonesia
		...
		//$url='https://wa.me/'.$url;
		$_POST['seting']=!empty($_POST['seting'])?$_POST['seting']:array();
		@$settings =json_encode($_POST['seting']);
		...
		//$_POST['text_color']=str_replace('#','',$_POST['text_color']);
		//$_POST['background_color']=str_replace('#','',$_POST['background_color']);
       ...
       // $subtype = 'whatsapp_form';
		...
		$stmt = Database::$database->prepare("UPDATE `links` SET `url` = ?, `location_url` = ?, `settings` = ?, `text_color` = ?, `bg_color` = ? , `keterangan` = ? WHERE `link_id` = ?");
        $stmt->bind_param('sssssss', $url, $_POST['location_url'], $settings,$_POST['text_color'],$_POST['background_color'],$_POST['keterangan'], $_POST['link_id']);
        $stmt->execute();
        $stmt->close();
		
		
		
		Response::json('successfully updated', 'success', ['url' => url('link/' . $_POST['link_id'] . '?tab=links')]);
