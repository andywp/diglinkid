...
/* 	public function mail_send($array,$via,$id,$admin=true ){
	...
	public function notification(){
		...
		foreach($product as $r){
			...
			$trans_id=($r['transaction_id'] !='')?$r['transaction_id']:$r['id'];
			
			
			$cek=$this->httpGet($config['urlgopay'].$trans_id.'/status');
		 /* echo '<pre>';
				print_r($cek);
				
			echo '</pre>';   */
			if($cek->status_code==407){
			
			if($cek->payment_type=='credit_card'){
				/* echo '<pre>';
				print_r($cek);
				
			echo '</pre>'; */
				if(@$cek->channel_response_message =='Approved'){
					
					
					
					send_mail( $this->settings,explode(',', $this->settings->email_notifications->emails)/* 'andy.wijang@gmail.com' */, 'Transaction #'.$cek->order_id.' settlement Via credit card', sprintf($this->language->global->email_notifications->new_payment_body, $cek->gross_amount, 'IDR'), $test = true);
			
					$data=Database::$database->query("update payments set processor='Credit Card', status='Approved', transaction_id='".$cek->transaction_id."' , date_paid='".$cek->transaction_time."'  where id='".$cek->order_id."' "); 
	...
	public function finish(){
		...
		if($data->payment_type=='credit_card'){
			...
			//$status=($data->channel_response_message=='Approved')?''
				
			$data=Database::$database->query("update payments set processor='Credit Card', status='".@$data->channel_response_message."', transaction_id='".$data->transaction_id."' , date_paid='".$data->transaction_time."'  where id='".$_GET['order_id']."' ");
			
			
		}elseif($data->payment_type=='gopay'){
			$data=Database::$database->query("update payments set processor='".$data->payment_type."', status='".@$data->transaction_status."', transaction_id='".$data->transaction_id."' where id='".$_GET['order_id']."' ");
