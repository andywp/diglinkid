...
<body class="link-body <?= $link->design->background_class ?>" style="<?= ($aktive == 1)?$link->design->background_style:''; ?>">
	<?php if($aktive == 1){ ?>
	<?php if($link->settings->google_tag_manager !=''){ ?>
                ...
                <footer class="link-footer">
                    ...
                    <?php if($link->settings->display_branding): ?>
                        <?php if(isset($link->settings->branding, $link->settings->branding->name, $link->settings->branding->url) && !empty($link->settings->branding->name)): ?>
                            <a id="branding" target="_blank" href="<?= !empty($link->settings->branding->url) ? $link->settings->branding->url : '#' ?>" style="<?= $link->design->text_style ?>"><?= $link->settings->branding->name ?></a>
                        <?php else: ?>
